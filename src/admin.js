...
jQuery(document).ready(function ($) {
	...
	$(document).on('click', '.pagination-btn', function () {
		...
		if (!window.searchResultsData || page < 1) {
			return;
		}
	...
	function showImportPreview(data) {
		...
		if (data.count === 0) {
			...
			if (data.total_imported && data.total_imported > 0) {
				noResultsHtml +=
					'<p><strong>Total URLs in file:</strong> ' + data.total_imported + '</p>';
				noResultsHtml +=
					'<p><strong>URLs with zero instances:</strong> ' + data.filtered_out + '</p>';
			}
			noResultsHtml +=
				'<p>You may want to check your URLs or try importing a different file.</p>';
		...
		if (data.filtered_out && data.filtered_out > 0) {
			...
			previewHtml +=
				'<strong>Note:</strong> ' +
				data.filtered_out +
				' URLs were filtered out because they have zero instances on this site. ';
		...
		data.replacements.forEach(function (replacement, index) {
			...
			previewHtml +=
				'<td><a href="' +
				escapeHtml(replacement.from_url) +
				'" target="_blank">' +
				escapeHtml(replacement.from_url) +
				'</a></td>';
			previewHtml +=
				'<td><a href="' +
				escapeHtml(replacement.to_url) +
				'" target="_blank">' +
				escapeHtml(replacement.to_url) +
				'</a></td>';
			...
			previewHtml +=
				'<button type="button" class="button button-small button-secondary preview-locations-btn" data-index="' +
				index +
				'">Preview Locations</button>';
			previewHtml +=
				'<button type="button" class="button button-small button-secondary edit-replacement-btn" data-index="' +
				index +
			...
			previewHtml += '</tr>';
			previewHtml +=
				'<tr class="preview-row" data-index="' + index + '" style="display: none;">';
		...
		previewHtml +=
			'<p><strong>Review the replacements above. Click "Replace Single" to load each replacement into the form above for individual processing.</strong></p>';
	...
	function refreshImportPreview() {
		...
		if (fromUrl && toUrl) {
			window.importPreviewData.replacements = window.importPreviewData.replacements.filter(
	...
	function buildPaginatedResultsTable() {
		...
		if (totalPages > 1) {
			...
			if (currentPage > 1) {
				html +=
					'<button type="button" class="button button-secondary pagination-btn" data-page="' +
					(currentPage - 1) +
					'">← Previous</button>';
			}
			...
			if (startPage > 1) {
				html +=
					'<button type="button" class="button button-secondary pagination-btn" data-page="1">1</button>';
				if (startPage > 2) {
	...
	function showReplaceResults(data, fromUrl, toUrl) {
		const message =
			'Successfully replaced "' + escapeHtml(fromUrl) + '" with "' + escapeHtml(toUrl) + '".';
	...
	function loadLocationPreview(index) {
		if (!window.importPreviewData || !window.importPreviewData.replacements[index]) {
			return;
		}
	...
	function displayLocationPreview(data, $container) {
		...
		if (data.locations.length > 0) {
			...
			data.locations.forEach(function (location) {
				...
				previewHtml +=
					'<span class="location-count">' + location.count + ' instance(s)</span>';
				previewHtml += '</div>';
	...
	function editReplacement(index) {
		...
		$results
			.html(
				'<div class="single-results-summary info"><h4>ℹ Info</h4><p>Replacement loaded into Single URL Replacement form above.</p></div>'
			)
			.show();
	}
});
